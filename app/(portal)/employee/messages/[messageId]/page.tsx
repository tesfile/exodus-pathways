...
async function replyToEmployeeThreadAction(formData: FormData) {
  "use server";
  ...
  revalidatePath(`/employee/messages/${messageId}`);
  revalidatePath("/employee/messages");
}

export default async function EmployeeMessageDetailPage({ params }: PageProps) {
  ...
  return <MessageThreadView thread={thread} backHref="/employee/messages" action={replyToEmployeeThreadAction} />;
}
